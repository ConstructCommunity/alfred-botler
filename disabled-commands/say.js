...
module.exports = class say extends Command {
  ...
  async run(msg, { channel, _extra }) {
    ...
    if (msg.attachments.array().length === 1) {
      got(msg.attachments.first().url, async (error, response, body) => {
        if (error) {
          console.log(error);
          const _ = await msg.author.send('Sorry, there was an error with the embed file');
          msg.delete();
          return;
        }
        ...
        try {
          ...
          const _ = await msg.author.send('Sorry, there was an error with the JSON of your file');
          msg.delete();
          return;
        }
        ...
        try {
          if (_extra !== undefined) {
            const _ = await channel.send(_extra.join(' '), { embed: json });
            msg.delete();
          } else {
            const _ = await channel.send({ embed: json });
            msg.delete();
          }
        } catch (e) {
          const _ = await msg.send(`There is an error with the message: ${e.message}`);
          msg.delete();
        }
      });
    }
    ...
    // No attachments
      ...
      const _ = await channel.send(_extra.join(' '));
      msg.delete();
    }
  }
};
